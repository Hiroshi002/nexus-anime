...
Architecture Decision Records (`docs/architecture/adr/`) capture _why_ a decision was made. These documents describe _what_ the architecture is and _how_ to implement it. If a decision here conflicts with an ADR, the ADR wins until a new ADR is written.
