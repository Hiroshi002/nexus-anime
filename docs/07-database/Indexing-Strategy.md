...
| **Unique constraints are indexes.**        | A `UNIQUE` constraint creates a unique B-tree index — it enforces integrity _and_ speeds lookups. We use partial unique indexes to handle soft-delete correctly.      |
