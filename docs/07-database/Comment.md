...
| `chk_comments_parent_same_anime` | check | `parent_comment_id IS NULL OR anime_id = (SELECT anime_id FROM comments WHERE id = parent_comment_id)` — *enforced via application + trigger; see §2.7* |
