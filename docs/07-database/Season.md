...
A `season` groups episodes within a single anime (e.g. *Attack on Titan* Season 1, Season 2, …). Not every show has seasons — films, OVAs, and some flat series have a single implicit season. The schema supports both: an episode may reference a `season_id` or be orphaned to the show directly.
