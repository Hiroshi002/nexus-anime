...
|------|------|------------|
| `uq_continue_watching_user_anime` | partial unique | `UNIQUE (user_id, anime_id) WHERE deleted_at IS NULL` — *see note* |
...
- **One row per (user, anime), not per episode:** A user resumes a *show*, not an episode. The cursor points to the latest episode they were watching. This keeps the table small (~10M rows at 1M users × 10 active shows) and the home feed query simple.
